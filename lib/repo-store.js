...
function RepoStore(params = {}) {
  ...
  this.startMongo = async function () {
    try {
      ...
      await mongoose.connect(
        ...
        (err) => {
          if (!err) {
            loggerTracer.info(`Mongoose connect complete on`, {
              args: `${URL_MONGOOSE}`,
            });
          } else {
            loggerTracer.error(`Mongoose connect has error ${err}`);
          }
        }
      );
    } catch (err) {
      loggerTracer.error(`Start mongo has been error: ${err}`);
  ...
  this.stopMongo = async function () {
    try {
      ...
      loggerTracer.error(`Stop mongo has error: ${err}`);
      return Promise.reject(err);
    }
  };
  /**
  ...
  this.startMySql = async function () {
    try {
      ...
      await createConnection.connect((err) => {
        if (!err) {
          loggerTracer.info(`MySql connect complete on`, {
            args: `${URL_MYSQL}`,
          });
        }
      });
      await dataSequelizeStore.sequelize.authenticate().then((err) => {
        if (!err) {
          loggerTracer.info(`Connection Sequelize has been successfully`);
        }
      });
    } catch (err) {
      loggerTracer.error(`Start mysql has been error: ${err}`);
      return Promise.reject(err);
    }
  };
  /**
  ...
  this.stopMySql = async function () {
    try {
      ...
      await dataSequelizeStore.sequelize.close((err) => {
        if (!err) {
          loggerTracer.warn(`Sequelize has been close !!!`);
        }
      });
    } catch (err) {
      loggerTracer.error(`Stop mysql close has error: ${err}`);
      return Promise.reject(err);
    }
  };
  /**
  ...
  this.startGraphql = async function (app, server, pathServer) {
    try {
      ...
      loggerTracer.info(`Graphql connect complete on`, {
        args: `${pathServer}${apolloServer.graphqlPath}`,
      });
    } catch (err) {
      loggerTracer.error(`Start graphql has been error: ${err}`);
      return Promise.reject(err);
    }
  };
  /**
  ...
  this.stopGraphql = async function (server) {
    try {
      ...
      loggerTracer.error(`Stop graphql has been error: ${err}`);
      return Promise.reject(err);
    }
  };
  /**
