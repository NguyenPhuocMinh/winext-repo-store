      ...
      return new Promise((resolve, reject) => {
        const connect = mongoose.connect(URL_MONGOOSE, {
          ...
          if (!err) {
            ...
            loggerTracer.debug(chalk.blue(`Load mongoose by ${name}-${version} successfully!`));
          } else {
      ...
      if (!isEmpty(mysqlConfig) && enableMySql) {
        await createConnection.connect(err => {
          if (!err) {
            ...
            loggerTracer.debug(chalk.blue(`Load mysql by ${name}-${version} successfully!`));
          }
        });
        ...
        loggerTracer.info(chalk.blue(`Not load mysql by ${name}-${version} successfully!`));
      }
    } catch (err) {
