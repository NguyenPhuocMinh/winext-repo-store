    ...
    return new Promise((resolve, reject) => {
      const connect = mongoose.connect(db_url, { useNewUrlParser: true }, (err) => {
        if (!err) {
          ...
          loggingFactory.data(`The mongoose connect complete ${db_url}`, { requestId: `${requestId}` });
        } else {
      ...
      .catch(err => {
        loggingFactory.error(`The Database Has Error : ${err}`, { requestId: `${requestId}` });
  ...
  this.disconnect = function (sandbox) {
    ...
    return new Promise(function (resolve, reject) {
      const disconnect = mongoose.connection.close((err) => {
        if (err) {
          loggingFactory.error(`Disconnection mongoose has error : ${err}`, { requestId: `${requestId}` });
          reject(err);
        } else {
          loggingFactory.data(`Mongoose disconnect ${db_url}`, { requestId: `${requestId}` });
        }
      })
      resolve(disconnect)
    })
      .then(() => {
        loggingFactory.info(`${name}-${version} is closed`, { requestId: `${requestId}` });
        process.exit(0);
      })
      .catch(err => {
        return Promise.reject(err);
