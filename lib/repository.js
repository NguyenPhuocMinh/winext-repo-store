      ...
      return new Promise((resolve, reject) => {
        const connect = mongoose.connect(URL_MONGOOSE, {
          ...
          if (!err) {
            ...
            loggerTracer.info(`Load mongoose by ${name}-${version} successfully!`);
          } else {
      ...
      if (enableMySql) {
        await createConnection.connect(err => {
          if (!err) {
            ...
            loggerTracer.info(`Load mysql by ${name}-${version} successfully!`);
          }
        });
        ...
        await dataSequelizeTrigger.sequelize.close(err => {
          if (!err) {
            ...
            loggerTracer.info(`Load sequelize by ${name}-${version} successfully!`);
          }
        });
      }
    } catch (err) {
