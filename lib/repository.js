...
function Repository(params = {}) {
  ...
  this.shutdownMySql = async function () {
    try {
      ...
      await sequelizeStore.sequelize.close(err => {
        if (!err) {
  ...
  this.sequelize = sequelizeTrigger().sequelize;
  this.Sequelize = sequelizeTrigger().Sequelize;
};

function sequelizeTrigger(params = {}) {
  const { sequelizeStoreTrigger } = params;

  return {
    sequelize: sequelizeStoreTrigger.sequelize,
    Sequelize: sequelizeStoreTrigger.Sequelize
  }
}
...
exports.sequelizeTrigger = sequelizeTrigger;
