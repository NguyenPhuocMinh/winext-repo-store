...
function Repository(params = {}) {
  ...
  this.startupMySql = function () {
    try {
      if (enableMySql) {
        return sequelizeStore.sequelize.authenticate()
          .then(err => {
            if (!err) {
        ...
        return sequelizeStore.sequelize.close();
      }
    } catch (err) {
  ...
  this.shutdownMySql = function () {
    try {
      return sequelizeStore.sequelize.close().then(err => {
        if (!err) {
          loggerFactory.debug(`MySql Sequelize has been close`, {
            requestId: `${requestId}`
          });
        }
      });
    } catch (err) {
      loggerFactory.debug(`MySql Sequelize has been close has error : ${err}`, {
        requestId: `${requestId}`,
        args: { err }
      });
      return Promise.reject(err);
   ...
   * sequelize
   */
  this.sequelize = sequelizeStore.createConnection(hostMySql, portMySql, userMysql, passwordMysql, databaseMySql, dialect, pool);
  /**
   * mongoose
   */
