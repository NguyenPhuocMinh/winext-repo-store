...
function Server() {
  ...
  this.start = function () {
    ...
    return new Promise((resolve, rejects) => {
      ...
      loggingFactory.debug(`The Server Start : ${serverInstance}`, { requestId: `${requestId}` });
      resolve(serverInstance)
    })
      .then(info => {
        loggingFactory.warn(`Start server complete : ${JSON.stringify(info, null, 1)}`, { requestId: `${requestId}` })
        return info
      })
      .catch(err => {
