...
function Server() {
  ...
  this.start = function () {
    ...
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portHttp, hostHttp, (err) => {
        ...
        loggingFactory.data(`The Server Start : ${protocolHttp}://${host}:${port}`);
      })
      resolve(serverInstance)
    })
      .then(info => {
        loggingFactory.warn(`Start server ${name} ${version} has complete`, { requestId: `${requestId}` });
        return info
      })
      .catch(err => {
        loggingFactory.warn(`Start server has error : ${err}`, { requestId: `${requestId}` })
  ...
  this.stop = function () {
    return new Promise(function (resolve, reject) {
      ...
      .catch(err => {
        loggingFactory.error(`The Server Stop Has Error : ${err}`, { requestId: `${requestId}` });
        return Promise.reject(err)
      })
  };
};
