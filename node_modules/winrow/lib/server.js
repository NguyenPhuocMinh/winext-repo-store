...
function Server() {
  ...
  this.start = function (sandbox) {
    ...
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portHttp, hostHttp, (err) => {
        ...
        loggingFactory.debug(`The server start With Path Server : ${pathServer}`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` });
        loggingFactory.info(`The server start With: ${protocolHttp}://${host}:${port}/${pathServer}`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` });
      })
      resolve(serverInstance)
    })
      .then(info => {
        loggingFactory.info(`Connect with library ${name.toUpperCase()}-${version} has complete`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` });
        return info
      })
      .catch(err => {
        loggingFactory.warn(`Start server has error : ${err}`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` })
  ...
  this.stop = function () {
    return new Promise(function (resolve, reject) {
      ...
      loggingFactory.warn(`Time Out : ${timeOut}`, { requestId: `${requestId}` });
      ...
      .then(() => {
        loggingFactory.warn('The Server Has Been Closed', { requestId: `${requestId}` });
        loggingFactory.warn(`The library ${name.toUpperCase()}-${version} has been disconnection`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` });
        process.exit();
      })
      .catch(err => {
        loggingFactory.error(`The Server Stop Has Error : ${err}`, { labelName: `[${name.toUpperCase()}]`, requestId: `${requestId}` });
        return Promise.reject(err)
      })
  };
};
